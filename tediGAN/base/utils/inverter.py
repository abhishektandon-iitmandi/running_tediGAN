...
class StyleGANInverter(object):
  ...
  def invert(self, image, num_viz=0):
    ...
    if self.mode == 'gen':
      ...
      # seed = 878
      x=generate.generate_images(None, network_path, [seed], 1, "const", out_dir, None, None)
      ...
      init_z = np.expand_dims(init_z.cpu().numpy(), axis=0)
      print(init_z.shape)
...
#     viz_results.append(self.G.postprocess(_get_tensor_value(x_init_inv))[0])
    ...
    for step in pbar:
      ...
      x_rec=generate.generate_images(None, network_path, None, 1, "const", out_dir, None, z)
      # print(x_rec.shape)
      # print(x_rec.type)
      ...
      if self.loss_reg_weight:
        ...
        # x_temp = x_rec
        # z_rec = self.E.net(torch.Tensor(x_temp).to(self.run_device)).view(1, *self.encode_dim)
